...
mod adversarial_fuzz {
    ...
    use crate::state::{
        FEE_GROWTH_GLOBAL_0, FEE_GROWTH_GLOBAL_1, POOL_STATE, POSITIONS, TICKS, TICK_BITMAP,
    };
    use choice_clmm_common::pool::{ExecuteMsg, FeeConfig, InstantiateMsg};
    use choice_clmm_common::types::AssetInfo;
    use choice_clmm_math::tick_math::{MIN_SQRT_RATIO};
    ...
    #[derive(Clone, Debug)]
    enum Op {
        Mint { oi: usize, lower: i32, upper: i32, liq: u128 },
        SwapIn { oi: usize, zero_for_one: bool, amt: u128 },
        SwapLimited { oi: usize, zero_for_one: bool, amt: u128, num: u32, den: u32 },
        SwapOut { oi: usize, zero_for_one: bool, amt_out: u128 },
        Burn { oi: usize, lower: i32, upper: i32, frac: u128 },
        Collect { oi: usize, lower: i32, upper: i32 },
    }

    /// Generate a single op from the PRNG given the current model.
    fn gen_op(st: &mut u64, positions: &BTreeMap<(usize, i32, i32), u128>) -> Op {
        match next(st) % 12 {
            0..=3 => {
                let oi = (next(st) % 4) as usize;
                let lower = -5000 + TICK_SPACING * (next(st) % 1000) as i32;
                let width = TICK_SPACING * (1 + next(st) % 400) as i32;
                let upper = lower + width;
                let liq = 1 + next(st) % 1_000_000_000;
                Op::Mint { oi, lower, upper, liq: liq as u128 }
            }
            4..=5 => {
                let oi = (next(st) % 4) as usize;
                let zero_for_one = (next(st) & 1) == 0;
                let amt = 1 + next(st) % 50_000_000;
                Op::SwapIn { oi, zero_for_one, amt: amt as u128 }
            }
            6 => {
                let oi = (next(st) % 4) as usize;
                let zero_for_one = (next(st) & 1) == 0;
                ...
                let (num, den) = if zero_for_one { (1 + next(st) % 99, 100) } else { (101 + next(st) % 99, 100) };
                Op::SwapLimited { oi, zero_for_one, amt: amt as u128, num: num as u32, den: den as u32 }
            }
            7 => {
                let oi = (next(st) % 4) as usize;
                let zero_for_one = (next(st) & 1) == 0;
                let amt_out = 1 + next(st) % 20_000_000;
                Op::SwapOut { oi, zero_for_one, amt_out: amt_out as u128 }
            }
            8..=9 => {
                if let Some((oi, l, u, _)) = pick(positions, st) {
                    let frac = 1 + next(st) % 4; // burn 1/4..1/1 of position
                    Op::Burn { oi, lower: l, upper: u, frac: frac as u128 }
                } else {
                    Op::SwapIn { oi: 0, zero_for_one: true, amt: 1 }
                }
            }
            _ => {
                if let Some((oi, l, u, _)) = pick(positions, st) {
                    Op::Collect { oi, lower: l, upper: u }
                } else {
                    Op::SwapIn { oi: 0, zero_for_one: false, amt: 1 }
                }
            }
        }
    }
        ...
        for m in &res.messages {
            if let CosmosMsg::Bank(BankMsg::Send { amount, .. }) = &m.msg {
                for coin in amount {
                    ...
                    let bal = if coin.denom == T0 { &mut *bal0 } else { &mut *bal1 };
                    assert!(
                        *bal >= amt,
    ...
    fn check_invariants(
        ...
        for ((_, l, u), liq) in positions.iter() {
            if *l <= cur && cur < *u {
                expected_active = expected_active.checked_add(*liq).expect("model active L overflow");
            }
        }
        assert_eq!(
            slot0.liquidity.u128(), expected_active,
            "INV-ACTL [{}]: contract active L {} != model {} (tick {})",
            ctx, slot0.liquidity.u128(), expected_active, cur
        );
        ...
        for (t, g) in exp_gross.iter() {
            let ti = contract_ticks.get(t).unwrap_or_else(|| {
                panic!("INV-GROSS [{}]: model expects tick {} but contract has none", ctx, t)
            });
            assert_eq!(
                ti.active_positions_count, *g,
                "INV-GROSS [{}]: tick {} gross {} != model {}", ctx, t, ti.active_positions_count, g
            );
            let ed = *exp_delta.get(t).unwrap();
            assert_eq!(
                ti.liquidity_delta, ed,
                "INV-DELTA [{}]: tick {} delta {} != model {}", ctx, t, ti.liquidity_delta, ed
            );
            assert!(ti.initialized, "INV-GROSS [{}]: tick {} present but not initialized", ctx, t);
        }
        ...
        for (t, ti) in contract_ticks.iter() {
            assert!(
                ...
                ctx, t, ti.active_positions_count
            );
        }

        // --- INV-NET: Σ liquidity_delta over all ticks telescopes to 0. ---
        let net: i128 = contract_ticks.values().map(|t| t.liquidity_delta).sum();
        assert_eq!(net, 0, "INV-NET [{}]: Σ liquidity_delta = {} (must be 0)", ctx, net);

        // --- INV-BMAP: bit set IFF a TICKS entry exists. ---
        // entry ⇒ bit
        for t in contract_ticks.keys() {
            let (word, bit) = word_bit(*t, TICK_SPACING);
            let w = TICK_BITMAP.may_load(storage, word).unwrap().unwrap_or_default();
            assert!(
        ...
        for (word, w) in words.iter() {
            assert!(!w.is_zero(), "INV-BMAP [{}]: empty bitmap word {} not pruned", ctx, word);
            for b in 0u32..256 {
                if bit_set(*w, b) {
    ...
    fn instantiate_pool(deps: &mut cosmwasm_std::OwnedDeps<impl Storage, MockApi, impl cosmwasm_std::Querier>, factory: &Addr) {
        instantiate(
            deps.as_mut(),
            mock_env(),
    ...
    fn apply_op(
        deps: &mut cosmwasm_std::OwnedDeps<cosmwasm_std::MemoryStorage, MockApi, cosmwasm_std::testing::MockQuerier>,
        ...
        match op {
            Op::Mint { oi, lower, upper, liq } => {
                let funds = vec![Coin::new(Uint128::new(big), T0), Coin::new(Uint128::new(big), T1)];
                let msg = ExecuteMsg::Mint { lower_tick: *lower, upper_tick: *upper, amount: Uint128::new(*liq) };
                if let Ok(res) = execute(deps.as_mut(), env.clone(), message_info(&owners[*oi], &funds), msg) {
                    settle(bal0, bal1, &funds, &res, ctx);
                    *positions.entry((*oi, *lower, *upper)).or_insert(0) += *liq;
                }
            }
            Op::SwapIn { oi, zero_for_one, amt } => {
                let denom = if *zero_for_one { T0 } else { T1 };
                let funds = vec![Coin::new(Uint128::new(*amt), denom)];
                let msg = ExecuteMsg::SwapExactInput { minimum_amount_out: Uint128::zero(), recipient: None, deadline: None };
                if let Ok(res) = execute(deps.as_mut(), env.clone(), message_info(&owners[*oi], &funds), msg) {
                    settle(bal0, bal1, &funds, &res, ctx);
                }
            }
            Op::SwapLimited { oi, zero_for_one, amt, num, den } => {
            ...
            Op::SwapOut { oi, zero_for_one, amt_out } => {
            ...
            Op::Burn { oi, lower, upper, frac } => {
                if let Some(liq) = positions.get(&(*oi, *lower, *upper)).copied() {
                    if liq > 0 {
                        let burn = (liq / *frac).max(1).min(liq);
                        let msg = ExecuteMsg::Burn { lower_tick: *lower, upper_tick: *upper, amount: Uint128::new(burn) };
                        if let Ok(res) = execute(deps.as_mut(), env.clone(), message_info(&owners[*oi], &[]), msg) {
        ...
        if check {
            check_invariants(&deps.storage, positions, &deps.api, ctx);
        }
    }
    ...
    fn replay(ops: &[Op], check: bool) {
        let mut deps = mock_dependencies();
        let factory = deps.api.addr_make("factory");
        instantiate_pool(&mut deps, &factory);
        ...
        for (i, op) in ops.iter().enumerate() {
            ...
            apply_op(&mut deps, &env, &owners, &mut bal0, &mut bal1, &mut positions, op, &ctx, check);
        }
    }

    /// Binary-search shrink: find the shortest prefix of `ops` that still panics.
    fn shrink_and_report(seed: u64, ops: &[Op]) -> ! {
        ...
        while lo < hi {
            ...
            let panicked = std::panic::catch_unwind(|| replay(&prefix, true)).is_err();
            if panicked {
                hi = mid;
            } else {
                lo = mid + 1;
        ...
        eprintln!("\n==== ADVERSARIAL FUZZ FAILURE (seed {seed}) ====");
        eprintln!("Minimal failing prefix is {lo} ops:");
        for (i, op) in ops[..lo].iter().enumerate() {
            eprintln!("  op#{i}: {op:?}");
        }
        eprintln!("Reproduce: replay(&MINIMAL_PREFIX, true)");
        ...
        let payload = std::panic::catch_unwind(move || replay(&prefix, true)).unwrap_err();
        let msg = payload
            .downcast_ref::<String>()
            .cloned()
    ...
    fn run(seed: u64, steps: usize) {
        ...
        for _ in 0..steps {
            let op = gen_op(&mut st, &model);
            if let Op::Mint { oi, lower, upper, liq } = &op {
        ...
        let result = std::panic::catch_unwind(|| replay(&ops, true));
        if result.is_err() {
            shrink_and_report(seed, &ops);
        }
    }
    ...
    fn fuzz_full_drain_cleanup() {
        for s in 0..20u64 {
            let seed = 0xDEAD_0000u64.wrapping_add(s.wrapping_mul(0x9E37_79B9));
            let mut deps = mock_dependencies();
            let factory = deps.api.addr_make("factory");
            instantiate_pool(&mut deps, &factory);
            let owners: Vec<Addr> = (0..4).map(|i| deps.api.addr_make(&format!("lp{i}"))).collect();
            ...
            let mut st = seed;
            for i in 0..STEPS {
                let op = gen_op(&mut st, &positions);
                let ctx = format!("seed{seed}:op#{i}");
                apply_op(&mut deps, &env, &owners, &mut bal0, &mut bal1, &mut positions, &op, &ctx, false);
            }
            ...
            for &(oi, l, u) in &keys {
                ...
                if liq > 0 {
                    let res = execute(
                        deps.as_mut(), env.clone(), message_info(&owners[oi], &[]),
                        ExecuteMsg::Burn { lower_tick: l, upper_tick: u, amount: Uint128::new(liq) },
                    ).unwrap();
            ...
            for &(oi, l, u) in &keys {
                let res = execute(
                    deps.as_mut(), env.clone(), message_info(&owners[oi], &[]),
                    ExecuteMsg::Collect {
                        recipient: owners[oi].to_string(), lower_tick: l, upper_tick: u,
                        amount0_requested: Uint128::MAX, amount1_requested: Uint128::MAX,
                    },
                ).unwrap();
            ...
            let slot0 = POOL_STATE.load(&deps.storage).unwrap();
            assert_eq!(slot0.liquidity.u128(), 0, "seed {seed}: residual active L after drain");
            let remaining_ticks = TICKS.range(&deps.storage, None, None, Order::Ascending).count();
            assert_eq!(remaining_ticks, 0, "seed {seed}: {remaining_ticks} ticks remain after full drain");
            let remaining_words = TICK_BITMAP.range(&deps.storage, None, None, Order::Ascending).count();
            assert_eq!(remaining_words, 0, "seed {seed}: {remaining_words} bitmap words remain after full drain");
            for &(oi, l, u) in &keys {
                let p = POSITIONS
                    .may_load(&deps.storage, (owners[oi].as_str(), l, u))
                    .unwrap();
                if let Some(p) = p {
                    assert_eq!(p.liquidity, 0, "seed {seed}: position {oi},{l},{u} retains L");
                    assert_eq!(p.tokens_owed_0, Uint128::zero(), "seed {seed}: owed0 not drained");
                    assert_eq!(p.tokens_owed_1, Uint128::zero(), "seed {seed}: owed1 not drained");
                }
            }
            // Whatever native dust remains is pool-favored (>= 0 trivially).
            let _ = (bal0, bal1, MIN_SQRT_RATIO, FEE_GROWTH_GLOBAL_0, FEE_GROWTH_GLOBAL_1);
        }
    }
}
