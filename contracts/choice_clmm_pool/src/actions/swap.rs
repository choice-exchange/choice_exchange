...
use crate::state::{FEE_GROWTH_GLOBAL_0, FEE_GROWTH_GLOBAL_1, POOL_CONFIG, POOL_STATE, TICKS};
...
pub fn execute_swap(
    ...
    let mut state_sqrt_price = slot0.sqrt_price;
    ...
    while !state_amount_remaining.is_zero() && state_sqrt_price != sqrt_price_limit_x96 {
        let (mut step_tick_next, step_initialized) = next_initialized_tick_in_chunk(
            deps.storage,
            state_tick,
    ...
    // 5. Save Global State
    slot0.sqrt_price = state_sqrt_price;
